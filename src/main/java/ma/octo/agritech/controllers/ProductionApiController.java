...
public class ProductionApiController {
    ...
    @Autowired
    public ProductionApiController(ProductionRepository productionRepository, ExploitationRepository exploitationRepository, CompaignRepository compaignRepository, ProductRepository productRepository, UserRepository userRepository) {
    ...
    public ResponseEntity<Production> Store(@RequestBody Production production, final Principal principal) {
        ...
        production.setUser(this.userRepository.findOneByUsername(principal.getName()));
